...
public class Gui_Manager_Town {

	public TownMasjid set_uni_prayers_times;

	public Gui_Manager_Town(TownMasjid townmasjid) {
		this.set_uni_prayers_times = townmasjid;
	}
		...
		if (whichCountDown == 1) { // Igama countdown for all prayers except
			...
			new CountDownTimer(timeDiffInMs, 1000) {
				...
				public void onTick(long millisUntilFinished) {
					...
					String hmsH = String.format("%02d",
							(millisUntilFinished / (60 * 60 * 1000)) % 24);
					catchUpTimeH1.setText("\t" + hmsH);
					String hmsM = String.format("%02d",
	...
	public void get_time_diff(Cursor c, Db_Manager_Town db_2) {
		...
		TextView shehriLbl = (TextView) this.set_uni_prayers_times
				.findViewById(R.id.shehriLbl);
		...
		gregorianTime.setText(CheckDate.dateformat_3().format(new Date()));
		...
		if (current_time.before(fajrAzan_time)) {
			...
			try {
				calendar_1.setTime(CheckDate.dateformat_2().parse(currentDate));
			...
			nextDay = CheckDate.dateformat_2().format(calendar_1.getTime());

			// parse nextDay with format_2 to get the right "date" syntax
			try {
				tomorrow_date = CheckDate.dateformat_2().parse(nextDay);
			} catch (ParseException e) {
				e.printStackTrace();
			}
		...
		if (current_time.before(fajr_time)) {
			...
			fajrBG.setBackgroundColor(Color.parseColor("#f1c40f"));
				...
				&& current_time.after(fajr_time)) {
			if (CheckDate.ifItIsFriday() == true) {
